tyrano.plugin.kag.menu = {
    ...
    loadGameData: function(data, options) {
      ...
      1 == this.kag.stat.is_strong_stop ? auto_next = "stop" : this.kag.stat.is_strong_stop = !1;
      this.kag.setTitle(this.kag.stat.title);
      if ("false" == options.bgm_over) {
        var map_se = this.kag.tmp.map_se;
        for (var key in map_se) map_se[key] && this.kag.ftag.startTag("stopse", {
          stop: "true",
          buf: key
        });
      ...
      if (this.kag.stat.cssload)
        for (file in this.kag.stat.cssload) {
          var style = '<link rel="stylesheet" href="' + file + "?" + Math.floor(1e7 * Math.random()) + '">';
    ...
    getSaveData: function() {
      ...
      for (var root = {
        ...
        }, save_slot_num = this.kag.config.configSaveSlotNum || 5, i = 0; i < save_slot_num; i++) {
        var json = {};
    ...
    screenFull: function() {
      var isFullScreen = document.webkitFullscreenElement || document.mozFullScreenElement || document.msFullscreenElement || document.fullScreenElement || !1,
        isEnableFullScreen = document.fullscreenEnabled || document.webkitFullscreenEnabled || document.mozFullScreenEnabled || document.msFullscreenEnabled || !1,
        elem = document.body;
      isEnableFullScreen && (elem.requestFullscreen ? isFullScreen ? document.exitFullscreen() : elem.requestFullscreen() : elem.webkitRequestFullscreen ? isFullScreen ? document.webkitExitFullscreen() : elem.webkitRequestFullscreen() : elem.mozRequestFullScreen ? isFullScreen ? document.mozCancelFullScreen() : elem.mozRequestFullScreen() : elem.msRequestFullscreen && (isFullScreen ? document.msExitFullscreen() : elem.msRequestFullscreen()))
    },
    test: function() {}
  };