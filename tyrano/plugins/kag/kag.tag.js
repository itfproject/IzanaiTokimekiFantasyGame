tyrano.plugin.kag.ftag = {
    tyrano: null, kag: null, array_tag: [], master_tag: {}, current_order_index: -1, init: function () {
        ...
        label_name ? this.nextOrderWithLabel(label_name) : this.nextOrderWithLabel("")
        ...
        if ("false" == this.kag.stat.flag_glyph) {
            $(".img_next").remove();
            this.kag.getMessageInnerLayer().find("p").append("<img class='img_next' src='./tyrano/images/system/nextpage.gif' />")
        ...
        if (this.kag.is_rider) {
            ...
            this.kag.log("**:" + this.current_order_index + "　line:" + tag.line);
            this.kag.log(tag)
        } if ("call" == tag.name && "make.ks" == tag.pm.storage || "make.ks" == this.kag.stat.current_scenario) {
            if (1 == this.kag.stat.flag_ref_page) {
                this.kag.tmp.loading_make_ref = !0;
                this.kag.stat.flag_ref_page = !1
            }
        } else if (1 == this.kag.stat.flag_ref_page && null == this.kag.getStack("call")) {
            //ここでコールスタックが溜まっている時（fixの間）は、ページリフレッシュをしないようにすれば良い
            this.kag.stat.flag_ref_page = !1;
            ...
            if (1 == this.kag.stat.is_hide_message) {
                ...
                this.master_tag[tag.name].log_join ? this.kag.stat.log_join = "true" : "text" == tag.name || (this.kag.stat.log_join = "false");
                this.checkCw(tag) && this.kag.layer.layer_event.show();
                if ("" != err_str) this.kag.error(err_str);
                else {
                    tag.pm._tag = tag.name;
                    this.master_tag[tag.name].start($.extend(!0, $.cloneObject(this.master_tag[tag.name].pm), tag.pm))
                }
            } else if (this.kag.stat.map_macro[tag.name]) {
                tag.pm = this.convertEntity(tag.pm);
                var pms = tag.pm, map_obj = this.kag.stat.map_macro[tag.name], back_pm = {};
    ...
    }, checkCw: function (tag) { return !!this.master_tag[tag.name].cw && (1 != this.kag.stat.is_script && 1 != this.kag.stat.is_html && 1 != this.kag.stat.checking_macro) }, nextOrderWithTag: function (target_tags) {
        try {
            this.current_order_index++;
            ...
            if ("" == target_tags[tag.name]) {
                if (this.master_tag[tag.name]) {
                    switch (tag.name) {
                        case "elsif": case "else": case "endif": var root = this.kag.getStack("if");
        ...
        for (key in pm) {
            ...
            if (val.length > 0 && "&" === c) pm[key] = this.kag.embScript(val.substr(1, val.length));
                ...
                var d = val.split("|");
                2 == d.length && (map_obj.pm[$.trim(d[0]).substr(1, $.trim(d[0]).length)] ? pm[key] = map_obj.pm[$.trim(d[0]).substr(1, $.trim(d[0]).length)] : pm[key] = $.trim(d[1]))
            }
        } return pm
    }, checkVital: function (tag) {
        var master_tag = this.master_tag[tag.name], err_str = "";
        if (!master_tag.vital) return "";
        for (var array_vital = master_tag.vital, i = 0;
            i < array_vital.length;
            i++)tag.pm[array_vital[i]] ? "" == tag.pm[array_vital[i]] && (err_str += "タグ「" + tag.name + "」にパラメーター「" + array_vital[i] + "」は必須です　\n") : err_str += "タグ「" + tag.name + "」にパラメーター「" + array_vital[i] + "」は必須です　\n";
        return err_str
        ...
        void 0 === pm && (pm = {});
        pm._tag = name;
        this.master_tag[name].start($.extend(!0, $.cloneObject(this.master_tag[name].pm), pm))
    }, nextOrderWithLabel: function (label_name, scenario_file) {
        this.kag.stat.is_strong_stop = !1;
        if (label_name) {
            -1 != label_name.indexOf("*") && (label_name = label_name.substr(1, label_name.length));
            this.kag.ftag.startTag("label", { label_name: label_name, nextorder: "false" })
        } if ("*savesnap" != label_name) {
            var that = this, original_scenario = scenario_file;
            label_name = label_name || "";
            scenario_file = scenario_file || this.kag.stat.current_scenario;
            label_name = label_name.replace("*", "");
            if (scenario_file != this.kag.stat.current_scenario && null != original_scenario) {
            ...
            var tmpsnap = this.kag.menu.snap, co = tmpsnap.current_order_index, cs = tmpsnap.stat.current_scenario;
        ...
        if ((scenario_file = scenario_file || this.kag.stat.current_scenario) != this.kag.stat.current_scenario || 1 == flag) {
            this.kag.layer.hideEventLayer();
            ...
            if ("yes" == auto_next) this.nextOrder();
                ...
                1 == this.kag.stat.is_skip && 0 == this.kag.stat.is_strong_stop && this.kag.ftag.nextOrder()
            } else "stop" == auto_next && this.kag.ftag.startTag("s", { val: {} })
        }
    }
};
tyrano.plugin.kag.tag.text = {
    cw: !0, pm: { val: "", backlog: "add" }, start: function (pm) {
        if (1 != this.kag.stat.is_script) if (1 != this.kag.stat.is_html) {
            var j_inner_message = this.kag.getMessageInnerLayer();
            j_inner_message.css({ "letter-spacing": this.kag.config.defaultPitch + "px", "line-height": parseInt(this.kag.config.defaultLineSpacing) + "px", "font-family": this.kag.config.userFace });
            this.kag.stat.current_message_str = pm.val;
            if ("true" == this.kag.stat.vertical) {
                if ("false" != this.kag.config.defaultAutoReturn) {
                    var j_outer_message = this.kag.getMessageOuterLayer(), limit_width = .8 * parseInt(j_outer_message.css("width"));
                    parseInt(j_inner_message.find("p").css("width")) > limit_width && (this.kag.stat.vchat.is_active ? this.kag.ftag.startTag("vchat_in", {}) : this.kag.getMessageInnerLayer().html(""))
                } this.showMessage(pm.val, pm, !0)
            } else {
                if ("false" != this.kag.config.defaultAutoReturn) {
                    j_outer_message = this.kag.getMessageOuterLayer();
                    var limit_height = .8 * parseInt(j_outer_message.css("height"));
                    parseInt(j_inner_message.find("p").css("height")) > limit_height && (this.kag.stat.vchat.is_active ? this.kag.ftag.startTag("vchat_in", {}) : this.kag.getMessageInnerLayer().html(""))
        ...
        if ("" != chara_name && "join" != pm.backlog || "" != chara_name && "true" == this.kag.stat.f_chara_ptext) {
            this.kag.pushBackLog("<br><span class='backlog_chara_name " + chara_name + "'>" + chara_name + "</span><br><img class='backlog_separator' src='data/image/backlog/line.png'><br><span class='backlog_text " + chara_name + "'>" + message_str + "</span>", "add");
            if ("true" == this.kag.stat.f_chara_ptext) {
                this.kag.stat.f_chara_ptext = "false";
                this.kag.stat.log_join = "true"
            }
        } else {
            var log_str = "<br><span class='backlog_chara_name " + chara_name + "'>" + chara_name + "</span><br><img class='backlog_separator' src='data/image/backlog/line.png'><br><span class='backlog_text " + chara_name + "'>" + message_str + "</span>";
        ...
        !function (jtext) {
            "" == jtext.html() && (isVertical ? jtext.append("<p class='vertical_text'></p>") : jtext.append("<p class=''></p>"));
            var current_str = "";
            if (0 != jtext.find("p").find(".current_span").length) {
                jtext.find("p").find(".current_span").find("span").css({ opacity: 1, visibility: "visible", animation: "" });
            ...
            if (that.kag.stat.vchat.is_active) {
                ...
                if ("" == chara_name) {
                    ...
                    var vchat_name_color = $.convertColor(that.kag.stat.vchat.chara_name_color), cpm = that.kag.stat.vchat.charas[chara_name];
                    cpm && "" != cpm.color && (vchat_name_color = $.convertColor(cpm.color));
                ...
                (j_span = that.kag.getMessageCurrentSpan()).css({ color: that.kag.stat.font.color, "font-weight": that.kag.stat.font.bold, "font-size": that.kag.stat.font.size + "px", "font-family": that.kag.stat.font.face, "font-style": that.kag.stat.font.italic });
            ...
            for (var append_str = "", i = 0;
                i < message_str.length;
                i++) {
                ...
                } append_str += " " == c ? "<span style='opacity:0'>" + c + "</span>" : flag_in_block ? "<span style='display:inline-block;opacity:0'>" + c + "</span>" : "<span style='opacity:0'>" + c + "</span>"
            } current_str += "<span>" + append_str + "</span>";
            that.kag.appendMessage(jtext, current_str);
            var append_span = j_span.children("span:last-child"), pchar = function (index) {
                var isOneByOne = 1 != that.kag.stat.is_skip && 1 != that.kag.stat.is_nowait && ch_speed >= 3;
                isOneByOne && function (index) {
                    if ("" != that.kag.stat.font.effect) {
                        append_span.children("span:eq(" + index + ")").on("animationend", (function (e) { $(e.target).css({ opacity: 1, visibility: "visible", animation: "" }) }));
                ...
                if (index <= message_str.length) {
                    ...
                    1 == that.kag.stat.is_click_text || 1 == that.kag.stat.is_skip || 1 == that.kag.stat.is_nowait ? pchar(++index) : setTimeout((function () { pchar(++index) }), ch_speed)
                } else {
                    that.kag.stat.is_adding_text = !1;
                    that.kag.stat.is_click_text = !1;
                    if ("true" != that.kag.stat.is_stop) if (isOneByOne) that.kag.stat.is_hide_message || that.kag.ftag.nextOrder();
                    else {
                        append_span.children("span").css({ visibility: "visible", opacity: "1" });
                        setTimeout((function () { that.kag.stat.is_hide_message || that.kag.ftag.nextOrder() }), parseInt(that.kag.config.skipSpeed))
                    }
                }
            };
            pchar(0)
...
tyrano.plugin.kag.tag.label = {
    pm: { nextorder: "true" }, start: function (pm) {
        if ("true" == this.kag.config.autoRecordLabel) {
            var sf_tmp = "trail_" + this.kag.stat.current_scenario.replace(".ks", "").replace(/\u002f/g, "").replace(/:/g, "").replace(/\./g, ""), sf_buff = this.kag.stat.buff_label_name, sf_label = sf_tmp + "_" + pm.label_name;
            if ("" != this.kag.stat.buff_label_name) {
                this.kag.variable.sf.record || (this.kag.variable.sf.record = {});
                var sf_str = "sf.record." + sf_buff, scr_str = sf_str + " = " + sf_str + "  || 0;" + sf_str + "++;";
                this.kag.evalScript(scr_str)
            } this.kag.variable.sf.record && (this.kag.variable.sf.record[sf_label] ? this.kag.stat.already_read = !0 : this.kag.stat.already_read = !1);
            this.kag.stat.buff_label_name = sf_label
        } "true" == pm.nextorder && this.kag.ftag.nextOrder()
    }
};
tyrano.plugin.kag.tag.config_record_label = {
    pm: { color: "", skip: "" }, start: function (pm) {
        if ("" != pm.color) {
            ...
            "true" == pm.skip ? this.kag.config.unReadTextSkip = "true" : this.kag.config.unReadTextSkip = "false";
...
tyrano.plugin.kag.tag.l = {
    cw: !0, start: function () {
        ...
        if (1 == this.kag.stat.is_skip) this.kag.ftag.nextOrder();
            ...
            } setTimeout((function () { 1 == that.kag.stat.is_wait_auto && (1 == that.kag.tmp.is_vo_play ? that.kag.tmp.is_vo_play_wait = !0 : that.kag.ftag.nextOrder()) }), auto_speed)
        }
    }
};
...
tyrano.plugin.kag.tag.graph = {
    vital: ["storage"], pm: { storage: null }, start: function (pm) {
        var jtext = this.kag.getMessageInnerLayer(), current_str = "";
        0 != jtext.find("p").find(".current_span").length && (current_str = jtext.find("p").find(".current_span").html());
        var storage_url = "";
        storage_url = $.isHTTP(pm.storage) ? pm.storage : "./data/image/" + pm.storage;
        this.kag.appendMessage(jtext, current_str + "<img src='" + storage_url + "' >");
        this.kag.ftag.nextOrder()
    }
};
tyrano.plugin.kag.tag.jump = {
    pm: { storage: null, target: null, countpage: !0 }, start: function (pm) {
        var that = this;
        setTimeout((function () { that.kag.ftag.nextOrderWithLabel(pm.target, pm.storage) }), 1)
    }
};
tyrano.plugin.kag.tag.r = {
    log_join: "true", start: function () {
        var that = this, j_inner_message = this.kag.getMessageInnerLayer(), txt = j_inner_message.find("p").find(".current_span").html() + "<br />";
        j_inner_message.find("p").find(".current_span").html(txt);
        setTimeout((function () { that.kag.ftag.nextOrder() }), 5)
    }
};
...
tyrano.plugin.kag.tag.cm = {
    start: function () {
        this.kag.ftag.hideNextImg();
        this.kag.stat.vchat.is_active ? this.kag.ftag.startTag("vchat_in", {}) : this.kag.layer.clearMessageInnerLayerAll();
        this.kag.stat.log_clear = !0;
...
tyrano.plugin.kag.tag.current = {
    pm: { layer: "", page: "fore" }, start: function (pm) {
...
tyrano.plugin.kag.tag.position = {
    pm: { layer: "message0", page: "fore", left: "", top: "", width: "", height: "", color: "", opacity: "", vertical: "", frame: "", radius: "", marginl: "0", margint: "0", marginr: "0", marginb: "0" }, start: function (pm) {
        var target_layer = this.kag.layer.getLayer(pm.layer, pm.page).find(".message_outer"), new_style = {};
        ...
        "" != pm.color && (new_style["background-color"] = $.convertColor(pm.color));
        "" != pm.radius && target_layer.css("border-radius", parseInt(pm.radius) + "px");
        if ("none" == pm.frame) {
        ...
        if ("" != pm.vertical) if ("true" == pm.vertical) {
        ...
        "0" != pm.marginl && (new_style_inner["padding-left"] = parseInt(pm.marginl) + "px");
        "0" != pm.margint && (new_style_inner["padding-top"] = parseInt(pm.margint) + "px");
        "0" != pm.marginr && (new_style_inner.width = parseInt(layer_inner.css("width")) - parseInt(pm.marginr) - parseInt(pm.marginl) + "px");
        "0" != pm.marginb && (new_style_inner.height = parseInt(layer_inner.css("height")) - parseInt(pm.marginb) - parseInt(pm.margint) + "px");
        this.kag.setStyles(layer_inner, new_style_inner);
        this.kag.ftag.nextOrder()
    }
};
tyrano.plugin.kag.tag.image = {
    pm: { layer: "base", page: "fore", visible: "", top: "", left: "", x: "", y: "", width: "", height: "", pos: "", name: "", folder: "", time: "", wait: "true", depth: "front", reflect: "", zindex: "1" }, start: function (pm) {
        var strage_url = "", folder = "", that = this;
        if ("base" != pm.layer) {
            var layer_new_style = {};
            "true" == pm.visible && "fore" == pm.page && (layer_new_style.display = "block");
            this.kag.setStyles(this.kag.layer.getLayer(pm.layer, pm.page), layer_new_style);
            if ("" != pm.pos) switch (pm.pos) {
                case "left": case "l": pm.left = this.kag.config["scPositionX.left"];
                    break;
                case "left_center": case "lc": pm.left = this.kag.config["scPositionX.left_center"];
                    break;
                case "center": case "c": pm.left = this.kag.config["scPositionX.center"];
                    break;
                case "right_center": case "rc": pm.left = this.kag.config["scPositionX.right_center"];
                    break;
                case "right": case "r": pm.left = this.kag.config["scPositionX.right"]
            }folder = "" != pm.folder ? pm.folder : "fgimage";
            strage_url = $.isHTTP(pm.storage) ? pm.storage : "./data/" + folder + "/" + pm.storage;
            var img_obj = $("<img />");
            ...
            if ("" != pm.time) {
                ...
                "back" == pm.depth ? this.kag.layer.getLayer(pm.layer, pm.page).prepend(img_obj) : this.kag.layer.getLayer(pm.layer, pm.page).append(img_obj);
                img_obj.animate({ opacity: 1 }, parseInt(pm.time), (function () { "true" == pm.wait && that.kag.ftag.nextOrder() }));
            ...
            var new_style = { "background-image": "url(" + (strage_url = $.isHTTP(pm.storage) ? pm.storage : "./data/" + folder + "/" + pm.storage) + ")", display: "none" };
...
tyrano.plugin.kag.tag.freeimage = {
    vital: ["layer"], pm: { layer: "", page: "fore", time: "", wait: "true" }, start: function (pm) {
        var that = this;
        if ("base" != pm.layer) {
            0 == pm.time && (pm.time = "");
            if ("" != pm.time) {
...
tyrano.plugin.kag.tag.free = {
    vital: ["layer", "name"], pm: { layer: "", page: "fore", name: "", wait: "true", time: "" }, start: function (pm) {
        var that = this;
        if ("base" != pm.layer) {
            0 == pm.time && (pm.time = "");
            if ("" != pm.time) {
                if (!(j_obj = (j_obj = this.kag.layer.getLayer(pm.layer, pm.page)).find("." + pm.name)).get(0) && "true" == pm.wait) {
                    that.kag.ftag.nextOrder();
                    return
                } var cnt = 0, s_cnt = j_obj.length;
                j_obj.animate({ opacity: 0 }, parseInt(pm.time), (function () {
                ...
                (j_obj = (j_obj = this.kag.layer.getLayer(pm.layer, pm.page)).find("." + pm.name)).remove();
                that.kag.ftag.nextOrder()
            }
        } else {
            var j_obj;
...
tyrano.plugin.kag.tag.ptext = {
    vital: ["layer", "x", "y"], pm: { layer: "0", page: "fore", x: 0, y: 0, vertical: "false", text: "", size: "", face: "", color: "", italic: "", bold: "", align: "left", edge: "", shadow: "", name: "", time: "", width: "", zindex: "9999", overwrite: "false" }, start: function (pm) {
        var that = this;
        ...
        var font_new_style = { color: pm.color, "font-weight": pm.bold, "font-style": pm.fontstyle, "font-size": pm.size + "px", "font-family": pm.face, "z-index": "999", text: "" };
        if ("" != pm.edge) {
        ...
        if ("true" == pm.overwrite && "" != pm.name && $("." + pm.name).length > 0) {
        ...
        if ("" != pm.time) {
            ...
            tobj.animate({ opacity: 1 }, parseInt(pm.time), (function () { that.kag.ftag.nextOrder() }))
        } else {
            target_layer.append(tobj);
            this.kag.ftag.nextOrder()
        }
...
tyrano.plugin.kag.tag.mtext = {
    vital: ["x", "y"], pm: { layer: "0", page: "fore", x: 0, y: 0, vertical: "false", text: "", size: "", face: "", color: "", italic: "", bold: "", shadow: "", edge: "", name: "", zindex: "9999", width: "", align: "left", fadeout: "true", time: "2000", in_effect: "fadeIn", in_delay: "50", in_delay_scale: "1.5", in_sync: "false", in_shuffle: "false", in_reverse: "false", wait: "true", out_effect: "fadeOut", out_delay: "50", out_scale_delay: "", out_sync: "false", out_shuffle: "false", out_reverse: "false" }, start: function (pm) {
        ...
        "fix" == pm.layer && tobj.addClass("fixlayer");
        target_layer.append(tobj);
        for (key in pm) "true" == pm[key] ? pm[key] = !0 : "false" == pm[key] && (pm[key] = !1);
        tobj.textillate({
            loop: pm.fadeout, minDisplayTime: pm.time, in: { effect: pm.in_effect, delayScale: pm.in_delay_scale, delay: pm.in_delay, sync: pm.in_sync, shuffle: pm.in_shuffle, reverse: pm.in_reverse, callback: function () { 0 == pm.fadeout && 1 == pm.wait && that.kag.ftag.nextOrder() } }, out: {
                effect: pm.out_effect, delayScale: pm.out_delay_scale, delay: pm.out_delay, sync: pm.out_sync, shuffle: pm.out_shuffle, reverse: pm.out_reverse, callback: function () {
                    tobj.remove();
...
tyrano.plugin.kag.tag.link = {
    pm: { target: null, storage: null }, start: function (pm) {
        ...
        var _target = pm.target, _storage = pm.storage, that = TYRANO;
...
tyrano.plugin.kag.tag.wait = {
    vital: ["time"], pm: { time: 0 }, start: function (pm) {
        var that = this;
        this.kag.stat.is_strong_stop = !0;
...
tyrano.plugin.kag.tag.quake = {
    vital: ["time"], pm: { count: 5, time: 300, timemode: "", hmax: "0", vmax: "10", wait: "true" }, start: function (pm) {
        var that = this;
        "0" != pm.hmax ? $("." + this.kag.define.BASE_DIV_NAME).effect("shake", { times: parseInt(pm.count), distance: parseInt(pm.hmax), direction: "left" }, parseInt(pm.time), (function () { "true" == pm.wait && that.kag.ftag.nextOrder() })) : "0" != pm.vmax && $("." + this.kag.define.BASE_DIV_NAME).effect("shake", { times: parseInt(pm.count), distance: parseInt(pm.vmax), direction: "up" }, parseInt(pm.time), (function () { "true" == pm.wait && that.kag.ftag.nextOrder() }));
        "false" == pm.wait && that.kag.ftag.nextOrder()
    }
};
tyrano.plugin.kag.tag.font = {
    pm: {}, log_join: "true", start: function (pm) {
        this.kag.setMessageCurrentSpan();
        ...
        pm.effect && ("none" == pm.effect ? this.kag.stat.font.effect = "" : this.kag.stat.font.effect = pm.effect);
        pm.effect_speed && (this.kag.stat.font.effect_speed = pm.effect_speed);
        pm.edge && ("none" == pm.edge || "" == pm.edge ? this.kag.stat.font.edge = "" : this.kag.stat.font.edge = $.convertColor(pm.edge));
        pm.shadow && ("none" == pm.shadow || "" == pm.shadow ? this.kag.stat.font.shadow = "" : this.kag.stat.font.shadow = $.convertColor(pm.shadow));
        this.kag.ftag.nextOrder()
    }
};
tyrano.plugin.kag.tag.deffont = {
    pm: {}, start: function (pm) {
        ...
        pm.italic && (this.kag.stat.default_font.italic = $.convertItalic(pm.italic));
        pm.effect && ("none" == pm.effect ? this.kag.stat.default_font.effect = "" : this.kag.stat.default_font.effect = pm.effect);
        pm.effect_speed && (this.kag.stat.default_font.effect_speed = pm.effect_speed);
        pm.edge && ("none" == pm.edge || "" == pm.edge ? this.kag.stat.default_font.edge = "" : this.kag.stat.default_font.edge = $.convertColor(pm.edge));
        pm.shadow && ("none" == pm.shadow || "" == pm.shadow ? this.kag.stat.default_font.shadow = "" : this.kag.stat.default_font.shadow = $.convertColor(pm.shadow));
        this.kag.ftag.nextOrder()
    }
};
tyrano.plugin.kag.tag.delay = {
    pm: { speed: "" }, log_join: "true", start: function (pm) {
...
tyrano.plugin.kag.tag.layopt = {
    vital: ["layer"], pm: { layer: "", page: "fore", visible: "", left: "", top: "", opacity: "", autohide: !1, index: 10 }, start: function (pm) {
        if ("message" == pm.layer) {
        ...
        if ("" != pm.visible) if ("true" == pm.visible) {
...
tyrano.plugin.kag.tag.ruby = {
    vital: ["text"], pm: { text: "" }, log_join: "true", start: function (pm) {
        var str = pm.text;
...
tyrano.plugin.kag.tag.locate = {
    pm: { x: null, y: null }, start: function (pm) {
...
tyrano.plugin.kag.tag.button = {
    pm: { graphic: "", storage: null, target: null, ext: "", name: "", x: "", y: "", width: "", height: "", fix: "false", savesnap: "false", folder: "image", exp: "", prevar: "", visible: "true", hint: "", clickse: "", enterse: "", leavese: "", clickimg: "", enterimg: "", auto_next: "yes", role: "" }, start: function (pm) {
        var target_layer = null;
        "" != pm.role && (pm.fix = "true");
        "false" == pm.fix ? (target_layer = this.kag.layer.getFreeLayer()).css("z-index", 999999) : target_layer = this.kag.layer.getLayer("fix");
        var storage_url = "";
        storage_url = $.isHTTP(pm.graphic) ? pm.graphic : "./data/" + pm.folder + "/" + pm.graphic;
        var j_button = $("<img />");
        ...
        var that = TYRANO;
        !function () {
            var _target = pm.target, _storage = pm.storage, _pm = pm, preexp = that.kag.embScript(pm.preexp), button_clicked = !1;
            j_button.hover((function () {
                "" != _pm.enterse && that.kag.ftag.startTag("playse", { storage: _pm.enterse, stop: !0 });
                if ("" != _pm.enterimg) {
                    var enter_img_url = "";
                    enter_img_url = $.isHTTP(_pm.enterimg) ? _pm.enterimg : "./data/" + _pm.folder + "/" + _pm.enterimg;
                ...
                if ("" != _pm.enterimg) {
                    var enter_img_url = "";
                    enter_img_url = $.isHTTP(_pm.graphic) ? _pm.graphic : "./data/" + _pm.folder + "/" + _pm.graphic;
                    $(this).attr("src", enter_img_url)
                }
            }));
            j_button.click((function (event) {
                ...
                } if ("none" == that.kag.layer.layer_event.css("display") && 1 != that.kag.stat.is_strong_stop) return !1;
                if ("" != _pm.role) {
                    that.kag.stat.is_skip = !1;
                    "auto" != _pm.role && that.kag.ftag.startTag("autostop", { next: "false" });
                    if (!("save" != _pm.role && "menu" != _pm.role && "quicksave" != _pm.role && "sleepgame" != _pm.role || 1 != that.kag.stat.is_adding_text && 1 != that.kag.stat.is_wait)) return !1;
                    switch (_pm.role) {
                        case "save": that.kag.menu.displaySave();
                            break;
                        case "load": that.kag.menu.displayLoad();
                            break;
                        case "window": that.kag.layer.hideMessageLayers();
                            break;
                        case "title": that.kag.backTitle();
                            break;
                        case "menu": that.kag.menu.showMenu();
                            break;
                        case "skip": that.kag.ftag.startTag("skipstart", {});
                            break;
                        case "backlog": that.kag.menu.displayLog();
                            break;
                        case "fullscreen": that.kag.menu.screenFull();
                            break;
                        case "quicksave": that.kag.menu.setQuickSave();
                            break;
                        case "quickload": that.kag.menu.loadQuickSave();
                            break;
                        case "auto": 1 == that.kag.stat.is_auto ? that.kag.ftag.startTag("autostop", { next: "false" }) : that.kag.ftag.startTag("autostart", {});
                            break;
                        case "sleepgame": j_button.trigger("mouseout");
...
tyrano.plugin.kag.tag.glink = {
    pm: { color: "black", font_color: "", storage: null, target: null, name: "", text: "", x: "auto", y: "", width: "", height: "", size: 30, graphic: "", enterimg: "", clickse: "", enterse: "", leavese: "", face: "" }, start: function (pm) {
        var target_layer = null;
        ...
        if ("auto" == pm.x) {
            var sc_width = parseInt(this.kag.config.scWidth), center = Math.floor(parseInt(j_button.css("width")) / 2), first_left = Math.floor(sc_width / 2) - center;
            j_button.css("left", first_left + "px")
        } else "" == pm.x ? j_button.css("left", this.kag.stat.locate.x + "px") : j_button.css("left", pm.x + "px");
        "" == pm.y ? j_button.css("top", this.kag.stat.locate.y + "px") : j_button.css("top", pm.y + "px");
        $.setName(j_button, pm.name);
        this.kag.event.addEventElement({ tag: "glink", j_target: j_button, pm: pm });
        this.setEvent(j_button, pm);
        target_layer.append(j_button);
        target_layer.show();
        this.kag.ftag.nextOrder()
...
tyrano.plugin.kag.tag.clickable = {
    vital: ["width", "height"], pm: { width: "0", height: "0", x: "", y: "", border: "none", color: "", mouseopacity: "", opacity: "140", storage: null, target: null, name: "" }, start: function (pm) {
        ...
        this.kag.event.addEventElement({ tag: "clickable", j_target: j_button, pm: pm });
        this.setEvent(j_button, pm);
        ...
        !function () {
            ...
            var _pm = pm;
            if ("" != _pm.mouseopacity) {
                j_button.bind("mouseover", (function () { j_button.css("opacity", $.convertOpacity(_pm.mouseopacity)) }));
                j_button.bind("mouseout", (function () { j_button.css("opacity", $.convertOpacity(_pm.opacity)) }))
...
tyrano.plugin.kag.tag.glyph = {
    pm: { line: "nextpage.gif", layer: "message0", fix: "false", left: 0, top: 0 }, start: function (pm) {
        $(".glyph_image").remove();
        if ("true" == pm.fix) {
            var j_layer = this.kag.layer.getLayer(pm.layer), j_next = $("<img class='glyph_image' />");
...
tyrano.plugin.kag.tag.trans = {
    vital: ["time", "layer"], pm: { layer: "base", method: "fadeIn", children: !1, time: 1500 }, start: function (pm) {
        this.kag.ftag.hideNextImg();
        this.kag.stat.is_trans = !0;
        var that = this;
        $.countObj(this.kag.layer.map_layer_fore);
        "false" == pm.children && 0;
        var map_layer_fore = $.cloneObject(this.kag.layer.map_layer_fore), map_layer_back = $.cloneObject(this.kag.layer.map_layer_back);
        for (key in map_layer_fore) 1 != pm.children && key !== pm.layer || function () {
            var _key = key, layer_back = (map_layer_fore[_key], map_layer_back[_key]);
            if (-1 != _key.indexOf("message") && "false" == layer_back.attr("l_visible")) {
                0;
...
tyrano.plugin.kag.tag.bg = {
    vital: ["storage"], pm: { storage: "", method: "crossfade", wait: "true", time: 3e3, cross: "false" }, start: function (pm) {
        this.kag.ftag.hideNextImg();
        var that = this;
        ...
        this.kag.preload(storage_url, (function () {
            var j_old_bg = that.kag.layer.getLayer("base", "fore"), j_new_bg = j_old_bg.clone(!1);
            ...
            "true" == pm.cross && $.trans(pm.method, j_old_bg, parseInt(pm.time), "hide", (function () { j_old_bg.remove() }));
...
tyrano.plugin.kag.tag.bg2 = {
    vital: ["storage"], pm: { name: "", storage: "", method: "crossfade", wait: "true", time: 3e3, width: "", height: "", left: "", top: "", cross: "false" }, start: function (pm) {
        this.kag.ftag.hideNextImg();
        var that = this;
        0 == pm.time && (pm.wait = "false");
        var storage_url = "./data/bgimage/" + pm.storage;
        $.isHTTP(pm.storage) && (storage_url = pm.storage);
        this.kag.preload(storage_url, (function () {
            var j_old_bg = that.kag.layer.getLayer("base", "fore"), j_new_bg = j_old_bg.clone(!1), j_bg_img = $("<img />");
            j_bg_img.css("position", "absolute");
            var scWidth = parseInt(that.kag.config.scWidth), scHeight = parseInt(that.kag.config.scHeight), left = 0, top = 0;
...
tyrano.plugin.kag.tag.layermode = {
    vital: [], pm: { name: "", graphic: "", color: "", mode: "multiply", folder: "", opacity: "", time: "500", wait: "true" }, start: function (pm) {
        this.kag.ftag.hideNextImg();
        var that = this, blend_layer = null;
        ...
        "" != pm.color && blend_layer.css("background-color", $.convertColor(pm.color));
        "" != pm.opacity && blend_layer.css("opacity", $.convertOpacity(pm.opacity));
        ...
        "" != pm.graphic ? this.kag.preload(storage_url, (function () { blend_layer.fadeIn(parseInt(pm.time), (function () { "true" == pm.wait && that.kag.ftag.nextOrder() })) })) : blend_layer.fadeIn(parseInt(pm.time), (function () { "true" == pm.wait && that.kag.ftag.nextOrder() }));
        "false" == pm.wait && this.kag.ftag.nextOrder()
    }
};
tyrano.plugin.kag.tag.layermode_movie = {
    vital: ["video"], pm: { name: "", mode: "multiply", opacity: "", time: "500", wait: "false", video: "", volume: "", loop: "true", mute: "false", speed: "", fit: "true", width: "", height: "", top: "", left: "", stop: "false" }, start: function (pm) {
        this.kag.ftag.hideNextImg();
        var that = this, blend_layer = null, video = (blend_layer = $("<video class='layer_blend_mode blendlayer blendvideo' data-video-name='" + pm.name + "' data-video-pm='' style='display:none;position:absolute;width:100%;height:100%;z-index:99' ></video>")).get(0), url = "./data/video/" + pm.video;
        video.src = url;
        "" != pm.volume ? video.volume = parseFloat(parseInt(pm.volume) / 100) : video.volume = 0;
        ...
        "" != pm.height ? video.style.height = pm.height + "px" : "false" == pm.fit ? video.style.height = "100%" : video.style.height = "";
        ...
        blend_layer.css("mix-blend-mode", pm.mode);
        $("#tyrano_base").append(blend_layer);
        blend_layer.fadeIn(parseInt(pm.time), (function () { "true" == pm.wait && "true" == pm.loop && "true" != pm.stop && that.kag.ftag.nextOrder() }));
        "false" == pm.wait && "true" != pm.stop && this.kag.ftag.nextOrder()
    }
};
tyrano.plugin.kag.tag.free_layermode = {
    vital: [], pm: { name: "", time: "500", wait: "true" }, start: function (pm) {
        this.kag.ftag.hideNextImg();
        var that = this, blend_layer = {}, cnt = (blend_layer = "" != pm.name ? $(".layer_blend_" + pm.name) : $(".blendlayer")).length, n = 0;
        if (0 != cnt) {
