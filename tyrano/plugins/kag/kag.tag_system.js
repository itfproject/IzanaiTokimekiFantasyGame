...
tyrano.plugin.kag.tag.clearvar = {
	...
	start: function(pm) {
		"" == pm.exp ? this.kag.clearVariable() : this.kag.evalScript("delete " +
			pm.exp);
...
tyrano.plugin.kag.tag.close = {
	...
	close: function() {
		...
		"undefined" != typeof require && void 0 !== require("nw.gui") && require(
...
tyrano.plugin.kag.tag.call = {
	...
	start: function(pm) {
		...
		null == pm.target && null != pm.storage ? this.kag.ftag.nextOrderWithIndex(-
			1, pm.storage) : this.kag.ftag.nextOrderWithLabel(pm.target, pm.storage)
	}
};
tyrano.plugin.kag.tag.return = {
	start: function() {
		var pm = this.kag.getStack("call");
		if (pm.caller && pm.caller.storage && "make.ks" == pm.caller.storage && 1 ==
			this.kag.tmp.loading_make_ref) {
			this.kag.stat.flag_ref_page = !0;
			this.kag.tmp.loading_make_ref = !1
		}
		var auto_next = pm.auto_next;
		this.kag.popStack("call");
		this.kag.ftag.nextOrderWithIndex(pm.index, pm.storage, void 0, void 0,
			auto_next)
		...
		//returnする時にwaitを解除して、ゲームが完全に停止するのを防ぐ
	}
};
...
tyrano.plugin.kag.tag.autoload = {
	...
	start: function(pm) {
		$.getStorage(this.kag.config.projectID + "_tyrano_auto_save", this.kag.config
			.configSave);
...
tyrano.plugin.kag.tag.edit = {
	...
	start: function(pm) {
		var j_text = $("<input class='text_box form' name='" + pm.name +
			"' type='text' value='' />");
		...
		"true" == pm.autocomplete ? j_text.attr("autocomplete", "on") : j_text.attr(
			"autocomplete", "off");
...
tyrano.plugin.kag.tag.preload = {
	...
	start: function(pm) {
		...
		if ("object" == typeof storage && storage.length > 0)
			for (var sum = 0, i = 0; i < storage.length; i++) that.kag.preload(storage[
				i], (function() {
				sum++;
				if (storage.length == sum) {
					"true" == pm.wait && that.kag.layer.showEventLayer();
					that.kag.ftag.nextOrder()
				}
			}));
...
tyrano.plugin.kag.tag.dialog = {
	...
	finish: function(pm) {
		"" != pm.storage || "" != pm.target ? this.kag.ftag.startTag("jump", pm) :
			this.kag.ftag.nextOrder()
	}
};
...
tyrano.plugin.kag.tag.loadcss = {
	...
	start: function(pm) {
		var file = pm.file,
			style = '<link rel="stylesheet" href="' + file + "?" + Math.floor(1e7 *
				Math.random()) + '">';
...
tyrano.plugin.kag.tag.save_img = {
	...
	start: function(pm) {
		...
		storage_url = $.isHTTP(pm.storage) ? pm.storage : "./data/" + folder + "/" +
			pm.storage;
...
tyrano.plugin.kag.tag.pushlog = {
	...
	start: function(pm) {
		"true" == pm.join ? this.kag.pushBackLog(pm.text, "join") : this.kag.pushBackLog(
			pm.text, "add");
		this.kag.ftag.nextOrder()
	}
};
...
tyrano.plugin.kag.tag.check_web_patch = {
	...
	checkPatch: function(obj, pm) {
		var that = this;
		void 0 === this.kag.variable.sf._patch_version && this.kag.evalScript(
			"sf._patch_version=" + this.kag.config.game_version);
		parseFloat(this.kag.variable.sf._patch_version) < parseFloat(obj.version) ?
			$.confirm("新しいアップデートが見つかりました。Ver:" + parseFloat(obj.version) + "「" + obj.message +
				"」<br />アップデートを行いますか？", (function() {
					alert("アップデートを行います。完了後、自動的にゲームは終了します。");
					var http = require("http"),
						fs = require("fs"),
						file = obj.file,
						url = $.getDirPath(pm.url) + file; - 1 != url.indexOf("https") && (
						http = require("https"));
